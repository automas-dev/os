...
int io_fs_file_close(io_fs_file_t * file) {
    ...
    return file->file_close_fn(file->fs->fs_data, file->file_data);
}
...
size_t io_fs_file_read(io_fs_file_t * file, char * buff, size_t count) {
    ...
    return file->file_read_fn(file->fs->fs_data, file->file_data, buff, count);
}
...
size_t io_fs_file_write(io_fs_file_t * file, const char * buff, size_t count) {
    ...
    return file->file_write_fn(file->fs->fs_data, file->file_data, buff, count);
}
...
size_t io_fs_file_seek(io_fs_file_t * file, int offset, int origin) {
    ...
    return file->file_seek_fn(file->fs->fs_data, file->file_data, offset, origin);
}
...
size_t io_fs_file_tell(io_fs_file_t * file) {
    ...
    return file->file_tell_fn(file->fs->fs_data, file->file_data);
}
